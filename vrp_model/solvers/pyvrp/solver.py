...
from vrp_model.solvers.options import LOG_PATH, MSG, SEED, TIME_LIMIT, merge_solver_options
...
def _export_name(row: dict, idx: int, prefix: str) -> str:
    lab = row["label"]
    if lab is not None:
        return lab
...
def _add_resolved_edges(
    ...
    travel_edges: dict[tuple[int, int], TravelEdgeAttrs],
    n: int,
    *,
    use_euclidean: bool,
) -> None:
...
class PyVRPSolver(Solver):
    ...
    def build_solver_model(self, model: Model) -> PyVRPModelLike:
        ...
        _add_resolved_edges(
            ...
            cast(dict[tuple[int, int], TravelEdgeAttrs], model._travel_edges),
            n_nodes,
        ...
        for vi, vehicle in enumerate(model._vehicles):
            ...
            vname = _export_name(vehicle, vi, "vehicle")
            pm.add_vehicle_type(
                num_available=1,
                capacity=cap,
                start_depot=sd_obj,
    ...
    def call_solver(self, pm: PyVRPModelLike) -> tuple[object, PyVRPResultLike]:
        ...
        result = cast(PyVRPResultLike, raw)
        best = result.best
        if best.is_feasible():
            return SolveStatus.FEASIBLE, result
        return SolveStatus.INFEASIBLE, result
    ...
    def find_solution_values(self, model: Model, result: PyVRPResultLike) -> Solution:
        ...
        unassigned = [Job(model, nid) for nid in job_ids if nid not in assigned]

        return Solution(
            routes=routes_out,
            cost=float(best.distance()),
    ...
    def _run(self, model: Model, options: dict) -> tuple[object, Solution]:
        ...
        raw_status, result = self.call_solver(pm)
        solution = self.find_solution_values(model, result)
        return raw_status, solution
